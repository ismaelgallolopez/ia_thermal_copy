    ...
    for j in range(ny):
        for i in range(nx):
            ...
            if id in interfaces:
                ...
                GLii = 0
                if i+1 < nx:
                    if id in heaters or id+1 in heaters:
                        parameter = 0.75
                    else:
                        parameter = 1
                    K_rows.append(id)
                    K_cols.append(id+1)
                    K_data.append(-parameter*GLx)
                    GLii += parameter*GLx
                if i-1 >= 0:
                    if id in heaters or id-1 in heaters:
                        parameter = 0.5
                    ...
                    GLii += parameter*GLx
                if j+1 < ny:
                    if id in heaters or id+nx in heaters:
                        parameter = 0.5
                    else:
                        parameter = 1
                    K_rows.append(id)
                    K_cols.append(id+nx)
                    K_data.append(-parameter*GLy)
                    GLii += parameter*GLy
                if j-1 >= 0:
                    if id in heaters or id-nx in heaters:
                        parameter = 0.5
...
##############################################################
n_entradas_test = 1000
n_entradas_train = 1000
nodos_lado = 13

n_entradas = n_entradas_test+n_entradas_train
...
for i in range(n_entradas):
    ...
    resultados,_,_ = PCB_case_2(T_interfaces=interfacesAleatorias[i],Q_heaters=potenciasAleatorias[i],Tenv=TenvAleatorias[i],display=False)
...
dataset_test = PCBDataset(T_interfaces[:n_entradas_test,:,:],Q_heaters[:n_entradas_test,:,:],T_env[:n_entradas_test,:,:],output[:n_entradas_test,:,:],
                 T_interfaces_mean,T_interfaces_std,Q_heaters_mean,
                 Q_heaters_std,T_env_mean,T_env_std,output_mean,
                 output_std)

dataset_train = PCBDataset(T_interfaces[n_entradas_test:,:,:],Q_heaters[n_entradas_test:,:,:],T_env[n_entradas_test:,:,:],output[n_entradas_test:,:,:],
                 T_interfaces_mean,T_interfaces_std,Q_heaters_mean,
                 Q_heaters_std,T_env_mean,T_env_std,output_mean,
                 output_std)
...
torch.save(dataset, os.path.join(dir_path,"Datasets",'PCB_dataset_finetuning.pth'))
torch.save(dataset_test, os.path.join(dir_path,"Datasets",'PCB_dataset_test_finetuning.pth'))
torch.save(dataset_train, os.path.join(dir_path,"Datasets",'PCB_dataset_train_finetuning.pth'))

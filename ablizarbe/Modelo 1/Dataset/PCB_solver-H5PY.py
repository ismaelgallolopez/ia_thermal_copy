...
# Abre el archivo HDF5 (o lo crea si no existe)
with h5py.File('dataset_simetrico.h5', 'w') as f:
    # Supongamos que queremos llamar a la función 3 veces
    for i in range(n_entradas):
        ...
        # Crear un grupo para cada conjunto de resultados
        grupo = f.create_group(f'entrada{i+1}')
        
        # Guardar las matrices en el grupo
        grupo.create_dataset('potencias', data=potencias)
        grupo.create_dataset('interfaces', data=interfaces)
        grupo.create_dataset('resultados', data=resultados.reshape(nodos_lado,nodos_lado))

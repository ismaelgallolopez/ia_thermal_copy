...
dataset = torch.load('PCB_dataset_tipico.pth')
...
class EncoderPotencias(nn.Module):
    def __init__(self):
        super(EncoderPotencias, self).__init__()
        self.conv1 = nn.Conv2d(2, 16*capasmult, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(16*capasmult, 32*capasmult, kernel_size=3, padding=1)  
        self.conv3 = nn.Conv2d(32*capasmult, 16*capasmult, kernel_size=3, padding=1)  
        self.conv4 = nn.Conv2d(16*capasmult, 4*capasmult, kernel_size=3, padding=1) 
        self.pool = nn.MaxPool2d(2, 2) 
        self.adaptativepool = nn.AdaptiveAvgPool2d((8,8))
        self.linear1 = nn.Linear(4*2*2*capasmult, 256)
...
encoder2 = EncoderPotencias() 

# Enviar los modelos al dispositivo correcto
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
encoder, general_decoder, residual_decoder, encoder2 = encoder.to(device), general_decoder.to(device), residual_decoder.to(device), encoder2.to(device)


# Definir los optimizadores
optimizer = optim.Adam(list(encoder.parameters()) + list(general_decoder.parameters()) + list(residual_decoder.parameters()) + list(encoder2.parameters()), lr=0.0001)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if avg_test_loss < last_test_loss:
            ...
            torch.save(encoder2.state_dict(), base_path2.format("EncoderPotencias"))
...
# Cargar los modelos
model_path = 'modelos\modeloPrueba_EncoderPotencias.pth'


encoder2.load_state_dict(torch.load(model_path))

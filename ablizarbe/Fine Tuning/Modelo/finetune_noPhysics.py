...
class ResidualDecoder(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(13*13, 256)
        self.fc2 = nn.Linear(256, 512)
        self.fc3 = nn.Linear(512, 256)
        self.fc4 = nn.Linear(256, 13*13)
...
optimizer = optim.Adam(list(model.parameters()) + list(residual.parameters()), lr=0.0001)
...
for epoch in range(num_epochs):
    ...
    for batch, target, scalar in train_loader: 
        ...
        outputs = residual(outputs.view(outputs.size(0),13*13)).view(outputs.size(0),13,13)
        ...
        loss = 2*criterion(outputs, target) +  criterion(outputs_interfaces,T_interfaces) #+ loss_p*0.5
    ...
    if avg_test_loss < last_test_loss:
        ...
        torch.save(residual.state_dict(), 'modelos\ residual.pth')
...
residual.load_state_dict(torch.load('modelos\ residual.pth'))
residual.eval()
